...
export default function EditQualification(props) {
  const{id,degree_name,year_of_pass,degree_from,document_url,employee_id}=props.data;
...
const [degree, setDegree] = useState(degree_name);
const [collage, setCollage] = useState(degree_from);
const [year, setYear] = useState(year_of_pass);
const [marks, setMarks] = useState(parseFloat(props.data.marks));
const [doc, setDoc] = useState(document_url);
...
//Common Functions For All
 ...
 if(degree.length<5){
 ...
 else if(doc!=document_url && doc.name.split('.').at(-1)!='pdf'){
  setAlertShow(true);
  ...
  axios({
    ...
    .then((response)=> {
      setAlertShow(true);
      setMessage("Qualification Information Updated");
